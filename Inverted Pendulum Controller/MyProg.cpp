...
void yamakawasTechnique(float *inputs, float x, float x_dot, float angle, float angle_dot)
{
    inputs[in_x] = (0.7 * x)     + (0.3 * x_dot);
    if (abs(inputs[in_x]) <= TOO_SMALL) inputs[in_x] = 0;       // To prevent a bug which occurred with float values below this value, shouldn't negatively affect algorithm.
    inputs[in_y] = (0.8 * angle) + (0.2 * angle_dot);
    if (abs(inputs[in_y]) <= TOO_SMALL) inputs[in_y] = 0;       // To prevent a bug which occurred with float values below this value, shouldn't negatively affect algorithm.

    // cout << "x: " << x << endl;
    // cout << "x_dot: " << x_dot << endl;
    // cout << "(0.7 * x): " << (0.7 * x) << endl;
    // cout << "(0.3 * x_dot): " << (0.3 * x_dot) << endl;
    // cout << "(0.7 * x) + (0.3 * x_dot): " << (0.7 * x) + (0.3 * x_dot) << endl;
...
void runInvertedPendulum()
{
    ...
    //-------------------------------------------------
    float startAngle = 0.5;
    float startX = -1;

    //Start somewhere on the left
    //Cart cart(-1.0, worldBoundary.y2 + 0.06);
    Cart cart(startX, worldBoundary.y2 + 0.125);
    //Rod rod(-1.0, worldBoundary.y2 + 0.06);
    Rod rod(startX, worldBoundary.y2 + 0.125 + startAngle);
    // Rod rod(-1.0, worldBoundary.y2 + 0.125 + 0.45);
    
    //---------------------------------------------------------------
    //***************************************************************
    //Set the initial angle of the pole with respect to the vertical
    prevState.angle = startAngle;  //initial angle  = 8 degrees
    // prevState.angle = 8.0 * (3.14/180);  //initial angle  = 8 degrees
    prevState.x = startX;                 // force start position to be same as initialised cart
    ...
    float maxAngleDot = -100.0;
    float maxX = -100.0;
    float maxXDot = -100.0;
    
    // Below was in attempt to fix low value float bug
        // setactivepage(page);
        // cleardevice();
        // drawInvertedPendulumWorld();

        // prevState.angle = 2.61801e-006;
        // prevState.angle_dot = -1.03692e-005;
        // prevState.angle_double_dot = 4.10974e-005;
        // prevState.x = 0.2034;
        // prevState.x_dot = -0.4748;
        // prevState.x_double_dot = -3.04784e-005;
        // // inputs[in_x] = -6.03408e-005;
        // // inputs[in_y] = 2.05682e-008;

        // yamakawasTechnique(inputs, prevState.x, prevState.x_dot, prevState.angle, prevState.angle_dot);
        ...
        //     cout << "in_x: " << inputs[in_x] << endl;
        //     cout << "in_y: " << inputs[in_y] << "\n" << endl;
        //     /*cout << "x: " << prevState.x << "; x_dot: " << prevState.x_dot << endl;
        //     cout << "theta: " << prevState.angle << "; theta_dot: " << prevState.angle_dot << endl;
        //     cout << "in_x: " << inputs[in_x] << "; in_y: " << inputs[in_y] << "\n" << endl;*/
        // }

        // maxAngle =    abs(newState.angle) > maxAngle        ? abs(newState.angle)     : maxAngle;
        // maxAngleDot = abs(newState.angle_dot) > maxAngleDot ? abs(newState.angle_dot) : maxAngleDot;
        // maxX =        abs(newState.x) > maxX                ? abs(newState.x)         : maxX;
        // maxXDot =     abs(newState.x_dot) > maxXDot         ? abs(newState.x_dot)     : maxXDot;
   ...
   cout << "Max angle dot = " << maxAngleDot << endl;
